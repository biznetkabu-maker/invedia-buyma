...
def is_plausible_model_code(code: str) -> bool:
    ...
    if _NUMERIC_ONLY.match(c) or _VOLUME_OR_SIZE.match(c):
        return False
    return not (not re.search(r"[A-Za-z]", c) or not re.search(r"\d", c))
...
def style_ids_equivalent(a: str | None, b: str | None) -> bool:
    ...
    return (
        len(na) >= _MIN_NORMALIZED_LEN
        and len(nb) >= _MIN_NORMALIZED_LEN
        and (na.endswith(nb) or nb.endswith(na))
    )


def scraped_matches_buyma_style(
    scraped_style_id: str | None,
