"""NET-A-PORTER (net-a-porter.com) 向けスクレイピングStrategy。

YNAP グループ（Yoox・The Outnet と同系列）。
CELINE、Saint Laurent、Maison Margiela 等のハイブランドを多数取り扱う。
"""
...
class NETAPORTERStrategy(ScraperStrategy):
    @property
    def domain(self) -> str:
    ...
    async def extract(self, page: Page) -> ExtractionResult:
        ...
        if price:
            result["price"] = price
        logger.debug("NET-A-PORTER extract: price=%s status=%s", price, stock)
        return result
