...
_OUT_OF_STOCK_TEXTS = {"sold out", "notify me", "out of stock", "currently unavailable"}
_IN_STOCK_TEXTS = {"add to bag", "add to cart", "in den warenkorb"}


class MYTHERESAStrategy(ScraperStrategy):
    @property
    def domain(self) -> str:
        return "mytheresa.com"
    ...
    async def extract(self, page: Page) -> ExtractionResult:
        ...
        if price:
            result["price"] = price
        logger.debug("MYTHERESA extract: price=%s status=%s", price, stock)
        return result

    async def _extract_price(self, page: Page) -> str | None:
        try:
            await page.wait_for_selector(
                "[class*='pricing'], [class*='priceValue'], [itemprop='price']",
                timeout=10_000,
            )
        except Exception as exc:
            logger.debug("mytheresa: %s", exc)
