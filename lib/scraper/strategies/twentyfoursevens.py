...
LVMH グループ運営のラグジュアリー多ブランドEC。
CELINE、Saint Laurent、Maison Margiela、Loewe 等を取り扱う。
"""
...
_IN_STOCK_TEXTS = {"add to cart", "add to bag", "add to basket"}


class TWENTYFOURSStrategy(ScraperStrategy):
    @property
    def domain(self) -> str:
        return "24s.com"
    ...
    async def extract(self, page: Page) -> ExtractionResult:
        ...
        if price:
            result["price"] = price
        logger.debug("24S extract: price=%s status=%s", price, stock)
        return result
