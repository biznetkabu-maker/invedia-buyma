"""
24S (24s.com) 検索ページの F12 相当解析（Step3 URL 探索）。

LVMH グループ運営。クラウド headless では Akamai 403 になりやすい → ローカル F12 必須。

商品 URL パターン（2026-05）:
  /en-us/{product-slug}_{SKU}
  例: /en-us/celine-small-cabas-tote-bag_CESS24BAG001

主ソース: JSON-LD Product / ItemList、HTML 内商品リンク
"""
...
_PRODUCT_PATH_RE = re.compile(
    r'"(/(?:en-[a-z]{2}/)?[a-z0-9]+(?:-[a-z0-9]+)+(?:_[A-Z0-9]+)?)"',
    re.I,
)
...
_NO_MATCH_RE = re.compile(
    r"no results|0 products|we couldn't find|we could not find|did not match|aucun résultat",
    re.I,
)
_24S_PRODUCT = re.compile(
    r"/(?:en-[a-z]{2}/)?[a-z0-9]+(?:-[a-z0-9]+)+(?:_[A-Z0-9]+)?$",
    re.I,
)
_EXCLUDE_PATH = re.compile(
    r"/(?:search|cart|login|account|wishlist|help)(?:/|$|\?)",
    re.I,
)


@dataclass
class TwentyFourSCatalogItem:
    name: str
    brand: str
    path: str
    sku: str
...
def is_valid_24s_product_url(url: str) -> bool:
    from urllib.parse import urlparse

    if not url or "24s.com" not in url.lower():
        return False
...
def _parse_json_ld_products(html: str) -> list[TwentyFourSCatalogItem]:
    out: list[TwentyFourSCatalogItem] = []
    seen: set[str] = set()
    ...
    def _add(name: str, brand: str, url: str, sku: str, source: str) -> None:
        ...
        if not is_valid_24s_product_url(full):
            return
        key = path.rstrip("/")
        if key in seen:
            return
        seen.add(key)
        sku_val = sku or _sku_from_path(path)
        out.append(
            TwentyFourSCatalogItem(
                name=name,
                brand=brand,
                path=path,
                sku=sku_val,
...
def _parse_embedded_links(html: str) -> list[TwentyFourSCatalogItem]:
    out: list[TwentyFourSCatalogItem] = []
    seen: set[str] = set()
    for m in _PRODUCT_HREF_RE.finditer(html or ""):
        url = m.group(0).split("?")[0]
        if not is_valid_24s_product_url(url):
            continue
        from urllib.parse import urlparse
        ...
        out.append(
            TwentyFourSCatalogItem(
                name=slug.replace("-", " ").replace("_", " ")[:120],
                brand="",
                path=path,
                sku=_sku_from_path(path),
                source="html_link",
            )
        )
    return out
...
def parse_24s_search_html(
    html: str,
    *,
    brand: str = "",
    require_brand_match: bool = True,
) -> list[TwentyFourSCatalogItem]:
...
def _score_item(
    ...
    *,
    ...
    if _has_category_intent(product_name):
        ...
        for hint in pos:
            ...
            if hint == "wallet" and "wallet" in blob:
                score += 25
            if hint == "bag" and any(k in blob for k in ("バッグ", "handbag", "tote", "bag")):
                score += 25
        for hint in neg:
    ...
    if brand and not _brand_matches(brand, item.brand, item.name, item.path):
        score -= 50
    if not is_valid_24s_product_url(item.url):
        score -= 100
...
def rank_24s_catalog_items(
    items: list[TwentyFourSCatalogItem],
    *,
    style_id: str = "",
    product_name: str = "",
    brand: str = "",
...
def merge_search_hits(
    catalog: list[TwentyFourSCatalogItem],
...
async def search_24s_product_urls(
    ...
    *,
    ...
    catalog = parse_24s_search_html(html, brand=brand)
