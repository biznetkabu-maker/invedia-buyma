#!/usr/bin/env python3
"""NET-A-PORTER 検索の F12 解析をローカルで記録（docs/NETAPORTER_SEARCH_F12.md 更新用）。"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="NET-A-PORTER 検索 F12 キャプチャ")
    ...
    from lib.supply_search.netaporter import lookup_netaporter_search_diagnose

    print(f"検索: {query}")
    print(f"型番: {args.style_id}  ブランド: {args.brand}")

    pname = query if args.query else f"{args.product} {args.style_id}".strip()
    urls, diag = lookup_netaporter_search_diagnose(
        query,
        brand=args.brand,
    ...
    if not urls:
        ...
        print(f"  2) py scripts\\capture_netaporter_f12.py {args.style_id} -v")
        print("  3) Chrome: https://www.net-a-porter.com/en-us/search?q=...")
    ...
    print("\nOK: docs/NETAPORTER_SEARCH_F12.md のテンプレに追記してください。")
    return 0
